...
Use `cli.commands` to add your own [Commander](https://github.com/tj/commander.js) commands to `npx payload`. Custom commands use the same API and lifecycle as Payload's built-in commands, so their arguments, options, validation, and help are handled by Commander.
...
Every CLI command that uses `run` supports being run on a schedule using cron syntax. Pass the `--cron` flag followed by the cron expression:
