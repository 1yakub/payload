...
The `config.bin` API has been removed. Custom commands are now registered through `config.cli.commands` and use the same [Commander](https://github.com/tj/commander.js) API as Payload's built-in commands.
...
export const createSeedCommand: CLICommand = ({ getPayload, run }) =>
  new Command('seed')
    .description('Seed the database.')
    .option('--clear', 'Delete existing documents first.')
    .action((options: { clear?: boolean }, command: Command) =>
      run({
        command,
        handler: async () => {
...
- Replace the exported `script(config)` function with a `CLICommand` that returns a Commander `Command`.
- Declare positional arguments and options with `.argument()` and `.option()` instead of reading `process.argv` directly.
- Use the provided `getPayload()` function so Payload can shut down database connections after the command finishes.
- Execute command work through `run()` to preserve `--cron`, exit code, and error handling behavior. Do not call `process.exit()` from the command.

Custom commands now appear in `payload --help`, receive Commander validation and help generation, and cannot use the same name or alias as a built-in command. Command names are case-sensitive. Because custom commands must be registered before Commander parses input, the Payload config is now loaded before displaying root CLI help.
